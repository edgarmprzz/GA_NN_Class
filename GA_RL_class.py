...
class GA_RL:
    def __init__(self,name):
        ...
        self.output_layer=1
        self.model=Model()
        # self.total_reward=Environment.Car()
    ...
    def save_agents_model(self):
        path='/home/edgar/gen_alg/agents_networks/Config_'+self.name+'_'+str(self.generation)+'.ini'
        self.model.save('/home/edgar/gen_alg/agents_networks/'+self.name+'_model_'+str(self.generation)+'.h5')
        config=ConfigObj(path)
        config.file=path
        config["Network"]={}
        for layers in range(len(self.layers)):
            config["Network"]["layers"]=self.layers[layers]
        for func in range(len(self.act_func)):
            config["Network"]["activation_functions"]=self.act_func[func]
        for opt in range(len(self.optmizer)):
            config["Network"]["optimizer"]=self.optmizer[opt]
        for loss in range(len(self.loss)):
            config["Network"]["loss"]=self.loss[loss]
        config.write()
